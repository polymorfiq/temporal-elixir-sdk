...
use temporalio_sdk_common::protos::coresdk::nexus::CancelNexusTask as CancelNexusTask;
...
impl From<nexus::nexus_task::Variant> for SdkNexusTaskVariant {
    fn from(external: nexus::nexus_task::Variant) -> Self {
        match external {
            nexus::nexus_task::Variant::Task(task) => {
                Self{task: Some(task.into()), ..Self::default()}
            }

            nexus::nexus_task::Variant::CancelTask(task) => {
                Self{cancel_task: Some(task.into()), ..Self::default()}
            }
        }
    }
}
...
impl From<PollNexusTaskQueueResponse> for SdkNexusPollTaskQueueResponse {
    fn from(external: PollNexusTaskQueueResponse) -> Self {
        Self {
            ...
            poller_group_infos: external.poller_group_infos.iter().map(|val| val.clone().into()).collect()
        }
    }
}
...
impl From<NexusRequestVariant> for SdkNexusRequestVariant {
    fn from(external: NexusRequestVariant) -> Self {
        match external {
            NexusRequestVariant::StartOperation(op) => {
                Self{start_operation: Some(op.into()), ..Self::default()}
            }

            NexusRequestVariant::CancelOperation(op) => {
                Self{cancel_operation: Some(op.into()), ..Self::default()}
            }
        }
    }
}
...
pub struct SdkNexusStartOperationRequest {
    ...
    pub callback: String,
    pub payload: Option<SdkActivationPayload>,
