  ...
  def next_poll_request(worker) do
    ...
    req = %WorkflowsSvcApi.PollWorkflowTaskQueueRequest{
      ...
      worker_version_capabilities:
        if(deploy_ver,
          do: %CommonApi.WorkerVersionCapabilities{
      ...
      deployment_options:
        if(deploy_ver,
          do: %DeploymentApi.WorkerDeploymentOptions{
            build_id: deploy_ver.build_id,
            worker_versioning_mode:
              EnumsApi.WorkerVersioningMode.value(:WORKER_VERSIONING_MODE_VERSIONED),
    ...
    with {:ok, polled} <-
           WorkflowsSvcApi.WorkflowService.Stub.poll_workflow_task_queue(
             Client.channel(worker.client),
             req
           ) do
      polled
    end
