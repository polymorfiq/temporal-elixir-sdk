defmodule Temporal.Protos.Temporal.Api.Workflowservice.V1.GetDeploymentReachabilityResponse do
  ...
  | 3 | **`last_update_time`** | `Google.Protobuf.Timestamp` | Reachability level might come from server cache. This timestamp specifies when the value |
  | 2 | **`reachability`** | `Temporal.Protos.Temporal.Api.Enums.V1.DeploymentReachability` |  |

  ### Additional Notes

    * `last_update_time` (`Google.Protobuf.Timestamp`): Reachability level might come from server cache. This timestamp specifies when the value
